...
function init() {
    ...
    out="$(
        for ((i = 1; i <= 12; i++)); do
            tput 'cup' "$((-cos[(i * 5) % 60] * centy / 10000 + centy))" "$((sin[(i * 5) % 60] * centx / 10000 + centx))"
            echo -n "$i"
        done
    )"
    ...
    out="$(
        for ((j = 0; j < 3; j++)); do
            for ((i = 0; i < size[j]; i++)); do
                tput 'cup' "${posy[last[j] * 10 + i]}" "${posx[last[j] * 10 + i]}"
                echo -n ' '
                tput 'cup' "${posy[curr[j] * 10 + i]}" "${posx[curr[j] * 10 + i]}"
                echo -n "${mark[j]}"
            done
        done
        tput 'cup' "$centy" "$centx"
        echo -n $'o\b'
    )"
    echo -n "$out"
